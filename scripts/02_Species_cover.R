...
data_cover_trim[,species := droplevels(as.factor(species))]
data_cover_trim[,cover_1 := cover / 100] # beta distrib needs cover between 0 and 1 

##looks like zero inflated beta
ggplot(data_cover_trim,aes( x = cover_1, fill = species))+geom_histogram(binwidth = 0.01)+theme_classic()+facet_wrap(~ treatment)
...
fg_line <- fg_line[,.N, by = fg_order]
fg_line[,n := cumsum(N)+0.5]
fg_line[,labelpos := n - N/2]
fg_line[,label := c("Tree","Shrub","Forb","Graminoid","Bryophyte")]

## plots! can be adapted to show full model, just zi, just mu
(figure_pred_sp <- ggplot(data_cover_trim_plot,aes( x = reorder(species_order,-as.numeric(fg_order_2)), y = cover,group = treatment,fill = treatment, color = treatment))+
  geom_vline(xintercept = fg_line[1:5,]$n,lty = 2, color = "grey30")+
  geom_point( size = 1,alpha = 0.5, position = position_jitterdodge(0.2,0.1,0.6),fill = NA)+
  geom_pointrange(aes(y = Estimate,ymin = Q2.5,ymax = Q97.5 ,x = reorder(species_order,-as.numeric(fg_order_2))),pred_sp,
                  color = "grey5",pch = 21,size = 0.5,lwd = 0.5,lineend = "round",position = position_dodge(0.6))+
  geom_text(data = fg_line,aes(x = labelpos,y= 37.5,label = label),inherit.aes = F,size = 4) +
  theme_classic()+
    scale_y_continuous(limits =  c(0,40),oob = scales::squish)+ ##squishing the very high cover, can be removed
